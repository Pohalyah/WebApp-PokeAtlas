...
const allTypes = Object.keys(chart) as PokemonTypeName[];

export function getTypeMatchups(defendingTypes: PokemonTypeName[]) {
  const weaknesses: PokemonResistanceItem[] = [];
  const resistances: PokemonResistanceItem[] = [];
  ...
  weaknesses.sort((left, right) => right.multiplier - left.multiplier);
  resistances.sort((left, right) => left.multiplier - right.multiplier);
